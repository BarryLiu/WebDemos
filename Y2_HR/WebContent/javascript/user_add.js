$(function() {
	$("form:first").submit(function() {
		...
		if (!reg1.test(userName.val())) {
			userName[0].select();
			$.messager.alert("消息提示", "用户名称格式不正确！不能有空格且长度必须大于等于5位！", 2000);
			return false;
		}
		if (!reg1.test(userPassword.val())) {
			userPassword[0].select();
			$.messager.alert("消息提示", "用户密码格式不正确！不能有空格且长度必须大于等于6位！", 2000);
			return false;
		}
		if (!reg2.test(userTrueName.val())) {
			userTrueName[0].select();
			$.messager.alert("消息提示", "真实姓名格式不正确！不能有空格且长度必须大于等于2位！", 2000);
			return false;
		}
		return true;
	});
